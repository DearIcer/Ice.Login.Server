...
public static class CaptchaUtil
{
    ...
    public static CaptchaResult GenerateCaptchaImage(string captchaCode, int width = 0, int height = 30)
    {
        ...
        SKColor[] c =
        {
            SKColors.Black, SKColors.Red, SKColors.DarkBlue, SKColors.Green, SKColors.Orange, SKColors.Brown,
            SKColors.DarkCyan, SKColors.Purple
        };
        ...
        for (var i = 0; i < captchaCode.Length; i++)
        {
            ...
            //字体
            var fontFamily = SKTypeface.FromFamilyName(fonts[findex], SKFontStyleWeight.Bold, SKFontStyleWidth.Normal,
        ...
        //随机画5条线
        for (var i = 0; i < 5; i++)
            canvas.DrawLine(new SKPoint(random.Next(image.Width), random.Next(image.Height)),
                new SKPoint(random.Next(image.Width), random.Next(image.Height)),
